...
def compute_tumor_diameter(
    ...
    """
    按 IBSI 定义计算肿瘤最大三维直径（mm）。

    在 0.5 等值面生成 ROI 表面网格，再计算凸包顶点之间的最大
    欧氏距离。保留 ``convex_hull_threshold`` 参数仅用于兼容旧调用方。
    ...
    return _maximum_3d_diameter_from_mask(mask, affine, voxel_count)


def _maximum_3d_diameter_from_mask(
    mask: np.ndarray,
    affine: np.ndarray,
    voxel_count: int,
) -> Dict:
    """IBSI Maximum 3D Diameter on the ROI surface mesh."""
    from scipy.spatial import ConvexHull, QhullError, distance
    from skimage.measure import marching_cubes

    # Padding closes ROIs that touch an image boundary. Subtracting one restores
    # vertices to the original voxel-index coordinate system before applying affine.
    padded = np.pad((mask > 0).astype(np.uint8), 1, mode="constant")
    vertices, _, _, _ = marching_cubes(padded, level=0.5)
    vertices -= 1.0
    world_vertices = nib.affines.apply_affine(affine, vertices)
    try:
        hull = ConvexHull(world_vertices)
    except QhullError:
        hull = ConvexHull(world_vertices, qhull_options="QJ")
    hull_points = world_vertices[hull.vertices]
    max_dist = float(distance.pdist(hull_points).max()) if len(hull_points) > 1 else 0.0
    return {
        "max_diameter_mm": round(max_dist, 2),
        "method": "ibsi_surface_mesh",
        "voxel_count": voxel_count,
        "hull_vertices_count": int(len(hull_points)),
        "note": "IBSI Maximum 3D Diameter",
    }
...
def analyze_tumor(
    ...
    try:
        ...
        # A failed geometric measurement must not discard a valid voxel volume.
        logger.exception("Diameter calculation failed for %s", tumor_mask_path)
        diameter_result = {
            "max_diameter_mm": None, "method": "failed",
            "note": f"Diameter unavailable: {type(exc).__name__}",
        }
    finally:
    ...
    return {
        ...
        "volume_cm3": round(vol_mm3 / 1000.0, 2),
        "tumor_voxels": tumor_voxels,
        "volume_cm3_unrounded": float(vol_mm3 / 1000.0),
    }
...
def analyze_liver_case(
    ...
    for tl in tumor_labels:
        ...
        # A supplied empty reference mask confirms no foreground; its filename
        # must not become a lesion instance in the structured result or report.
        if result.get("tumor_voxels") == 0:
            continue
    ...
    return {
        ...
        "total_tumor_volume_cm3": round(sum(t["volume_cm3_unrounded"] for t in tumor_results.values()), 2),
        "largest_tumor_volume_cm3": round(max((t["volume_cm3_unrounded"] for t in tumor_results.values()), default=0.0), 2),
        "volume_lesion_count": len(tumor_results),
        "volume_scope": "all_analyzed_tumor_masks",
    }
