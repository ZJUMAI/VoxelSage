...
class ReflectionEngine:
    ...
    ERROR_CATEGORIES = {
        "PORT_B_HTTP_404": {
            ...
            "severity": "high",
            "retryable": False,
            "description": "Port B 中不存在该病例或其处理结果",
            "common_causes": ["病例尚未处理", "病例 ID 不存在", "病例数据不可用"],
            "strategies": ["停止调用依赖该病例的技能", "明确说明证据不可用"]
        },
        "PORT_B_HTTP_500": {
            "type": "temporary",
            "severity": "medium",
    ...
    def suggest_recovery_strategy(
        ...
        # 病例级 404 对所有依赖该病例的技能都不可恢复。继续尝试子技能
        # 只会得到相同错误，因此应立即停止并向上层返回证据不可用。
        if analysis.get("category") == "missing_case":
            strategy["action"] = "skip"
            strategy["reasoning"] = "病例不存在或尚未处理，其他病例相关技能也无法取得证据"
            return strategy
