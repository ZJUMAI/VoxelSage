...
class MedicalKnowledgeBase:
    ...
    def validate_result_consistency(cls, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(tumor_results, dict):
            tumor_results = {}
        explicit_count = result.get("tumor_count")
        actual_tumor_count = (
            explicit_count
            if isinstance(explicit_count, int)
            else (len(tumor_results) if isinstance(tumor_results, dict) else 0)
        )
        ...
        for tumor_name, tumor_data in tumor_results.items():
            ...
            # 检查必需字段
            nested_diameter = tumor_data.get("diameter", {})
            diameter_value = tumor_data.get("max_diameter_mm")
            if diameter_value is None and isinstance(nested_diameter, dict):
                diameter_value = nested_diameter.get("max_diameter_mm")
            required_values = {
                "volume_cm3": tumor_data.get("volume_cm3"),
                "max_diameter_mm": diameter_value,
            }
    ...
    def extract_analysis_summary(cls, tool_store: Dict[str, Any], case_id: str) -> Dict[str, Any]:
        ...
        if isinstance(tumor_results, dict):
            explicit_count = liver_result.get("tumor_count")
            summary["tumor_count"] = (
                explicit_count if isinstance(explicit_count, int) else len(tumor_results)
            )
            ...
            for tumor_data in tumor_results.values():
                ...
                if diam is None and isinstance(tumor_data.get("diameter"), dict):
                    diam = tumor_data["diameter"].get("max_diameter_mm")
                diam = diam or 0
