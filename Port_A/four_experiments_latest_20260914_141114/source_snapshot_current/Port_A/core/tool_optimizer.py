...
class ToolOptimizer:
    ...
    def filter_redundant_tools(
        ...
        if case_id:
            ...
            check_cases = session.get("_active_case_ids") or session.get("case_ids") or list(tool_store.keys())
        ...
        for cid in check_cases:
            ...
            for skill_name, result in case_data.items():
                ...
                if isinstance(result, dict) and result and not result.get("_error") and result.get("status") not in {"error", "failed"}:
                    case_skills.add(skill_name)
            per_case_skills.append(case_skills)
        if per_case_skills:
            completed_skills = set.intersection(*per_case_skills)
        ...
        for tool in available_tools:
            ...
            for parent_skill, hierarchy in cls.SKILL_HIERARCHY.items():
                if parent_skill in completed_skills:
                    # A parent skill can return partial measurements. Keep child
                    # tools available unless every active case has its evidence.
                    if tool_name in hierarchy["includes"] and all(
                        cls.parent_has_measurement(tool_store[cid][parent_skill], tool_name)
                        for cid in check_cases
                    ):
    ...
    @staticmethod
    def parent_has_measurement(result: Dict[str, Any], tool_name: str) -> bool:
        # Aggregate maxima do not establish complete per-lesion measurements.
        # Vessel/distance tools may target structures absent from a partial result,
        # so leave them available; the model can still reuse returned evidence.
        if tool_name != "tumor_diameter":
            return False
        tumors = result.get("tumor_results")
        if not isinstance(tumors, dict) or not tumors:
            return False
        if result.get("tumor_count", len(tumors)) != len(tumors):
            return False
        for tumor in tumors.values():
            if not isinstance(tumor, dict):
                return False
            value = tumor.get("max_diameter_mm")
            if value is None and isinstance(tumor.get("diameter"), dict):
                value = tumor["diameter"].get("max_diameter_mm")
