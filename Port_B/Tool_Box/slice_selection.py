...
def select_largest_organ_slice_with_mask2d(
    ...
    """
    indices, sources = select_largest_organ_slice(organ_mask_path, volume_depth)
    if not indices:
        return [], {}, None
    ...
    mask_bin = mask > 0
    best_slice = indices[0]
    mask2d_u8 = mask_bin[:, :, best_slice].astype(np.uint8)
    return indices, sources, mask2d_u8


def select_topk_organ_slices_with_mask2d(
    organ_mask_path: str,
    volume_depth: int,
    top_k: int = 3,
    min_gap: int = 5,
) -> Tuple[List[int], Dict[int, str], List[np.ndarray]]:
    """
    Select top-K slices with the largest cross-sectional area of the organ mask,
    ensuring a minimum gap between selected slices to avoid redundancy.

    Args:
        organ_mask_path: path to organ segmentation NIfTI (.nii / .nii.gz)
        volume_depth: number of slices (D), used for sanity clipping
        top_k: number of slices to select (default: 3)
        min_gap: minimum slice index gap between selected slices (default: 5)

    Returns:
        indices: [slice_idx, ...] top-K slice indices sorted by area descending
        sources: {slice_idx: "largest_organ_cross_section"}
        mask2d_list: list of (H, W) uint8 2D masks for each selected slice
    """
    if not os.path.exists(organ_mask_path):
        return [], {}, []
    ...
    sources = {idx: "largest_organ_cross_section" for idx in selected}
    mask2d_list = [mask_bin[:, :, idx].astype(np.uint8) for idx in selected]
    return selected, sources, mask2d_list