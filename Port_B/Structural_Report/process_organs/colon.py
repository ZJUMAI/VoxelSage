...
def generate_colon_organ_report(colon_mask, spacing, ct=None):
    ...
    if vol is not None:
        vol_cm3 = vol / 1000.0
        _, vol_warning = validate_organ_volume("colon", vol_cm3)
...
def analyze_colon_lesion_mask(
    ...
    # ---- 计算结肠总体积供病灶比例校验 ----
    colon_voxels = int((colon_mask > 0.5).sum()) if colon_mask is not None else 0
    ...
    for comp_label in range(1, num + 1):
        ...
        _, lesion_warning = validate_lesion_volume(
            voxels, colon_voxels,
            lesion_desc=f"colon lesion {comp_label}",
        )
        # ------------------------------------------
