...
from utils.medical_commonsense import (
    validate_organ_volume,
    validate_lesion_volume,
    check_kidney_symmetry,
)
...
def generate_kidney_organ_report(ct, kidney_right, kidney_left, spacing):
    ...
    # ---- 医学常识校验：体积范围 ----
    vol_right_cm3 = vol_right / 1000.0 if vol_right is not None else None
    vol_left_cm3 = vol_left / 1000.0 if vol_left is not None else None

    _, warn_right = validate_organ_volume("kidney_right", vol_right_cm3)
    _, warn_left = validate_organ_volume("kidney_left", vol_left_cm3)
    _, warn_total = validate_organ_volume("kidney_total",
                                          vol_total / 1000.0 if vol_total else None)

    # ---- 医学常识校验：双侧对称性 ----
    _, warn_symmetry = check_kidney_symmetry(vol_left_cm3, vol_right_cm3)

    size_right = 'normal'
    size_left = 'normal'

    if vol_right is not None and vol_right_cm3 is not None:
        if vol_right_cm3 > (415.2 / 2):
            size_right = 'large'
        if warn_right:
            size_right = 'uncertain'
    if vol_left is not None and vol_left_cm3 is not None:
        if vol_left_cm3 > (415.2 / 2):
            size_left = 'large'
        if warn_left:
            size_left = 'uncertain'
    ...
    if vol_total is not None:
        if size_right == 'uncertain' or size_left == 'uncertain':
            text += "Volume measurement may be unreliable "
        elif size_right == 'normal' and size_left == 'normal':
            text += "Normal size "
        else:
        ...
        # ---- 附加医学警告 ----
        for w in [warn_right, warn_left, warn_total, warn_symmetry]:
            if w:
                text += w + "\n"
...
def analyze_kidney_lesion_mask(
    ...
    # ---- 计算肾脏总体积供病灶比例校验 ----
    kidney_voxels = int((union_kidney_mask > 0.5).sum())
    ...
    for comp_label in range(1, num + 1):
        ...
        # ---------- 病灶/器官体积比校验 ----------
        _, lesion_warning = validate_lesion_volume(
            voxels, kidney_voxels,
            lesion_desc=f"kidney {lesion_type} {comp_label}",
        )
        # ------------------------------------------
