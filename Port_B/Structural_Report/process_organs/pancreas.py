...
from utils.medical_commonsense import validate_organ_volume, validate_lesion_volume
...
def generate_pancreas_organ_report(ct, pancreas_mask, spacing, phase=None, spleen_hu=None):
    ...
    if vol is not None:
        ...
        # ---- 医学常识校验 ----
        vol_plausible, vol_warning = validate_organ_volume("pancreas", vol_cm3)
    ...
    if vol is not None:
        ...
        # ---- 附加医学警告 ----
        if not vol_plausible and vol_warning:
            text += vol_warning + "\n"
...
def analyze_pancreas_lesion_mask(
    ...
    # ---- 计算胰腺总体积供病灶比例校验 ----
    pancreas_voxels = int((pancreas_mask > 0.5).sum())
    ...
    for comp_label in range(1, num + 1):
        ...
        _, lesion_warning = validate_lesion_volume(
            voxels, pancreas_voxels,
            lesion_desc=f"pancreas {lesion_type} {comp_label}",
        )
        # ------------------------------------------
