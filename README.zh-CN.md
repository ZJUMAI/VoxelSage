...
| 方式 | 状态 | 适用场景 |
| --- | --- | --- |
| [本地源码部署](#快速开始) | 已支持 | 科研、开发和受控环境 |
| 在线演示 | **TODO：补充公开地址** | 无需本地安装即可体验界面 |
| Docker / 一键部署 | 暂未提供 | 可复现的自托管部署 |

使用 HTTPS 或远程部署时，请在构建前设置 `Frontend/.env.local` 中的三个
`VITE_*` 变量，并使用 `https://`、`wss://` 地址，或通过同源反向代理转发。

<!-- TODO(owner): 公开部署可用后替换在线演示占位。 -->
